...
**⚙ ตรวจอย่างไร (ตอนนี้ · จากโค้ด · line 495-513)**

1. ดู meta property="og:*" ของแต่ละหน้า — *Open Graph = ข้อมูลที่โชว์ตอนแชร์ลิงก์บน social/LINE*
2. เช็คว่ามีครบชุดจำเป็นไหม: og:title, og:description, og:image, og:type — *ครบชุดนี้ preview ถึงจะสมบูรณ์*
3. จัดระดับต่อหน้า: ไม่มี og เลย / มีแต่ขาดตัวจำเป็น / ครบ — *สะท้อนคุณภาพจริง ไม่ใช่แค่มี/ไม่มี 2 tag*

เกณฑ์ตัดสิน:
- มีหน้าที่ไม่มี og:* เลย → ผิดร้ายแรง (แชร์แล้วไม่มี preview)
- ทุกหน้ามี og แต่บางหน้าขาดตัวจำเป็น → ควรแก้ (preview ไม่สมบูรณ์)
- ทุกหน้ามีครบ title/description/image/type → ผ่าน

ข้อสังเกต: เช็คว่ามี tag ครบไหม ยังไม่ตรวจว่า og:image โหลดได้จริง/ขนาดถูก (1200×630) หรือเนื้อหา og ตรงกับหน้าจริง
