...
` + '```html\n<script type="application/ld+json">\n' + JSON.stringify({
  ...
  }, null, 2) + '\n</script>\n```\n_(domain/ชื่อ/description ดึงจากเว็บจริงแล้ว — ยืนยัน path โลโก้ให้ตรง และเพิ่ม address/telephone จริงใน Organization จะทำให้ entity สมบูรณ์ขึ้น)_');
...
function eeatFix(audit) {
  ...
  if (groups.article?.length) parts.push(`**Experience / Expertise (E) — ใส่ผู้เขียน (author) ในบทความ ${groups.article.length} บทความ**\n${list(groups.article)}\nวาง byline บนหน้า + Person schema (ผู้เขียนจริง) — ให้ Google/AI รู้ว่าใครเขียน มีความเชี่ยวชาญอะไร\n` + '```html\n<!-- บนหน้าบทความ ใต้หัวข้อ — แทนค่าใน <...> ด้วยข้อมูลจริงของบทความนั้น -->\n<p class="byline">โดย <a href="' + origin + '/<หน้าโปรไฟล์ผู้เขียน>"><ชื่อผู้เขียนจริง></a> · เผยแพร่ <time datetime="<YYYY-MM-DD>"><วันที่เผยแพร่></time></p>\n\n<script type="application/ld+json">\n' + JSON.stringify({ '@context': 'https://schema.org', '@type': 'Person', name: '<ชื่อผู้เขียนจริง>', jobTitle: '<ตำแหน่งงาน>', worksFor: { '@id': `${origin}/#org` }, url: `${origin}/<หน้าโปรไฟล์ผู้เขียน>` }, null, 2) + '\n</script>\n```\n_(domain ถูกแล้ว — เหลือแทนชื่อผู้เขียน/วันที่/path โปรไฟล์ ด้วยของจริง)_');
